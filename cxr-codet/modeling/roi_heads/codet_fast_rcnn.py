...
__all__ = ["CoDetFastRCNNOutputLayers"]


class CoDetFastRCNNOutputLayers(FastRCNNOutputLayers):
    @configurable
    def __init__(
            self,
        ...
        self.hard_neg_cap_weight = hard_neg_cap_weight
        self.add_image_box = add_image_box
        self.softmax_weak_loss = softmax_weak_loss
        self.debug = debug
        self.ws_proposal_num = ws_proposal_num
        self.g_size = g_size
        ...
        self.fc1 = nn.Linear(self.ws_proposal_num * (self.g_size-1), self.ws_proposal_num)
        self.fc2 = nn.Linear(self.ws_proposal_num, 1)
        ...
        self.weight_transform = nn.Sequential(
            ...
            self.fc2, nn.Flatten(start_dim=0), nn.Softmax(dim=0)
        )
    ...
    def from_config(cls, cfg, input_shape):
        ...
        ret.update({
            ...
            'hard_neg_cap_weight': cfg.MODEL.ROI_BOX_HEAD.HARD_NEG_CAP_WEIGHT,
            ...
            'ws_proposal_num': cfg.MODEL.ROI_BOX_HEAD.WS_NUM_PROPS,
            'g_size': cfg.DATALOADER.CONCEPT_GROUP_SIZE,
            'use_text_guidance': cfg.MODEL.ROI_BOX_HEAD.USE_TEXT_GUIDANCE
        })
    ...
    def sigmoid_cross_entropy_loss(self, pred_class_logits, gt_classes):
        ...
        if self.ignore_zero_cats and (self.freq_weight is not None):
            ...
            weight = weight * w.view(1, C).expand(B, C).to(pred_class_logits.device)
    ...
    def image_label_losses(self, predictions, proposals, image_labels, \
        ...
        if self.image_label_loss == 'concept_grouping':
            concepts = []
            for i in range(len(image_labels) // self.g_size):
                concept_sets = [set(image_labels[x]) for x in range(self.g_size * i, self.g_size * (i + 1))]
                concept_set_shared = list(set.intersection(*concept_sets))
                if not concept_set_shared:
                    print("concept_set_shared is empty")
                    continue
                concept_id = random.choice(concept_set_shared)
                for _ in range(self.g_size):
                    concepts.append([concept_id])
            full_image_labels = image_labels
            image_labels = concepts
        for idx, (score, labels, prop_score, p) in enumerate(zip(
            ...
            for i_l, label in enumerate(labels):
                ...
                elif self.image_label_loss == 'concept_grouping':
                    g_idx = idx // self.g_size
                    in_group_idx = idx - g_idx * self.g_size
                    g_proposals = list(proposals[g_idx * self.g_size: (g_idx + 1) * self.g_size])
                    # print(f"full_image_labels:{full_image_labels}")
                    # print(f"len_g:{len(g_proposals)}")
                    loss_i, ind = self._concept_grouping_loss(
                        score, label, g_proposals, in_group_idx, full_image_labels[idx])
                else:
                    assert 0
    ...
    def forward(self, x, ann_type='box', classifier_info=(None, None, None)):
        """
        ...
        if classifier_info[0] is not None:
            cls_scores = self.cls_score(x, ann_type, classifier=classifier_info[0])
            scores.append(cls_scores)
        else:
            cls_scores = self.cls_score(x, ann_type)
            scores.append(cls_scores)

        if classifier_info[2] is not None:
            cap_cls = classifier_info[2]
            if self.sync_caption_batch:
                caption_scores = self.cls_score(x, ann_type, classifier=cap_cls[:, :-1])
            else:
                caption_scores = self.cls_score(x, ann_type, classifier=cap_cls)
    ...
    def _caption_loss(self, score, classifier_info, idx, B):
        ...
        if self.sync_caption_batch:
            ...
            caption_target_copy = caption_target.clone()
            caption_target_copy[:, B * rank:B * (rank + 1)] = 1.
        else:
        ...
        if self.sync_caption_batch:
            ...
            if self.image_label_loss == 'concept_grouping':
                ng_mask = (caption_target_copy < 0.5).float()
                neg_loss = (caption_loss_img * ng_mask).sum()
                mg_mask = ((caption_target_copy - caption_target) > 0.5).float()
                hard_neg_loss = (caption_loss_img * mg_mask).sum()
                caption_loss_img = pos_loss + self.neg_cap_weight * neg_loss + self.hard_neg_cap_weight * hard_neg_loss
            else:
                ng_mask = (caption_target < 0.5).float()
                neg_loss = (caption_loss_img * ng_mask).sum()
    ...
    def _concept_grouping_loss(self, score, label, g_proposals, idx, full_label):
        loss = 0
        target = score.new_zeros(score.shape[1])
        target[label] = 1.
        proposals = [x.feat[:-1] for x in g_proposals] if self.add_image_box else [x.feat for x in g_proposals]
        # some images have fewer proposals, pad them with zero tensors
        for i in range(len(proposals)):
            n = len(proposals[i])
            if n < self.ws_proposal_num:
                comple = proposals[i].new_zeros((self.ws_proposal_num-n, proposals[i].shape[-1])).requires_grad_(False)
                proposals[i] = torch.cat([proposals[i], comple], dim=0)
        # print(f"ws:{self.ws_proposal_num}")
        # print(f"len_p:{len(proposals)}")
        simi_scores = self.compute_simi_scores(proposals, idx, label)
        # print(f"idx:{idx}")
        simi_weights = self.weight_transform(simi_scores)
        ind = simi_weights[:score.shape[0]].argmax().item()
        prototye = torch.sum(simi_weights.unsqueeze(dim=1) * proposals[idx], dim=0).reshape(1, 512)
        cls_score = self.cls_score.forward_without_linear_mapping(prototye)
        if self.use_fed_loss:
            class_num = score.shape[1] - 1
            appeared = get_fed_loss_inds(
                torch.tensor(full_label), num_sample_cats=50, C=class_num, weight=torch.ones(class_num))
            appeared_mask = score.new_zeros(class_num + 1)
            appeared_mask[appeared] = 1.
            appeared_mask[-1] = 1.
            appeared_mask = appeared_mask.float().to(score.device)
            cls_loss = F.binary_cross_entropy_with_logits(cls_score.squeeze(), target, reduction='none')
            loss += torch.sum(cls_loss * appeared_mask)
        else:
            loss += F.binary_cross_entropy_with_logits(cls_score.squeeze(), target, reduction='sum')

        return loss, ind

    def compute_simi_scores(self, proposals, idx, label):
        # print(f"label:{label}")
        if self.use_text_guidance:
            # print(1)
            class_weight = torch.abs(self.cls_score.zs_weight[:, label])
            class_weight = torch.div(class_weight, torch.sum(class_weight)) * len(class_weight)
        else:
            # print(0)
            class_weight = torch.ones(self.cls_score.zs_weight.shape[-1], device=proposals[0].device)
        # print(len(proposals))
        # print("-----------------------------------------------------------------------------------")
        ...
        # print("")
        # print("")
        anchor_proposals = proposals[idx]
        # print(f"initial_anchor_proposals:{anchor_proposals}")
        # print(f"len_a:{anchor_proposals.shape}")
        anchor_proposals = F.normalize(anchor_proposals, dim=-1)
        # print(f"normalized_anchor_proposals:{anchor_proposals}")
        support_proposals = proposals[0:idx] + proposals[idx + 1:]
        # print(f"initial_support_proposals:{support_proposals}")
        # print(f"len_s:{len(support_proposals)}")
        # print(idx)
        # print(len(proposals))
        support_proposals = torch.cat(support_proposals, dim=0)
        support_proposals = F.normalize(support_proposals, dim=-1)
        # print(f"normalized_support_proposals:{support_proposals}")

        # print(f"s{support_proposals.shape}")
        # print(f"a:{anchor_proposals.shape}")
        simi_scores = torch.einsum('ik,jk,k->ij', anchor_proposals, support_proposals, class_weight)
        # print(f"simi_scores:{simi_scores}")
        return simi_scores
