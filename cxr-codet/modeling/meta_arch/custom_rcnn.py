...
class CustomRCNN(GeneralizedRCNN):
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        if self.with_caption and 'caption' in ann_type:
            ...
            if isinstance(caps[0], dict): #
                caps = [c['caption'] if 'caption' in c else str(c) for c in caps] #
            tokenized = tokenizer(caps).to(self.device)
            caption_features = self.text_encoder(tokenized).float()
    ...
    def _sample_cls_inds(self, gt_instances, ann_type='box'):
        if ann_type == 'box':
            ...
            C = self.num_classes
            freq_weight_1 = torch.zeros(len(self.freq_weight))
            freq_weight_2 = torch.ones(C - len(self.freq_weight))
            freq_weight = torch.cat((freq_weight_1, freq_weight_2), dim=0)
    ...
    def preprocess_image(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        if 'square_size' in self.backbone.padding_constraints:
            ...
            if square_size >= max_size * 2 and square_size != 0:
                padding_constraints['square_size'] = square_size // 2
