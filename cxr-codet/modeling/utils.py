...
def reset_cls_test(model, cls_path, num_classes):
    ...
    zs_weight = torch.cat(
        ...
        dim=1) # D x (C + 1)
    if model.module.roi_heads.box_predictor.cls_score.norm_weight:
...
def reset_cls_infer(model, cls_path, num_classes):
    ...
    zs_weight = torch.cat(
        [zs_weight, zs_weight.new_zeros((zs_weight.shape[0], 1))],
        dim=1) # D x (C + 1)
    print(type(model.roi_heads.box_predictor))
    ...
    del predictor.cls_score.detection_weight
    predictor.cls_score.detection_weight = zs_weight
