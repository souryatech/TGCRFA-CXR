...
def _custom_train_loader_from_config(cfg, mapper=None, *, dataset=None, sampler=None):
    ...
    elif sampler_name == "MultiDatasetConceptSampler":
        sampler = MultiDatasetConceptSampler(
            dataset_dicts,
            dataset_ratio=cfg.DATALOADER.DATASET_RATIO,
            use_rfs=cfg.DATALOADER.USE_RFS,
            dataset_ann=cfg.DATALOADER.DATASET_ANN,
            repeat_threshold=cfg.DATALOADER.REPEAT_THRESHOLD,
            concept_group_size=cfg.DATALOADER.CONCEPT_GROUP_SIZE
        )
...
def get_detection_dataset_dicts_with_source( dataset_names, filter_empty=True, min_keypoints=0, proposal_files=None ):
    ...
    # print(MetadataCatalog.get('vindr_zeroshot_train').thing_classes)
    # for i in dataset_names:
    #     print(DatasetCatalog.get(i))
...
class MultiDatasetConceptSampler(Sampler):
    def __init__(
            self,
            dataset_dicts,
            dataset_ratio,
        ...
        self._rank = comm.get_rank()
        self._world_size = comm.get_world_size()
        self.group_size = concept_group_size

        self.dataset_ids = torch.tensor(
            [d['dataset_source'] for d in dataset_dicts], dtype=torch.long)

        self.catalog = [defaultdict(lambda: [[], []]) for _ in range(len(dataset_ratio))]

        for i, d in enumerate(dataset_dicts):
            if dataset_ann[int(d['dataset_source'])] == 'box':
                ids = set([x['category_id'] for x in d['annotations']])
                w, h = d["width"], d["height"]
                aspect_ratio_bucket_id = 0 if w > h else 1
                for id in ids:
                    self.catalog[d['dataset_source']][id][aspect_ratio_bucket_id].append(i)
            else:
                w, h = d["width"], d["height"]
                aspect_ratio_bucket_id = 0 if w > h else 1
                for id in d['pos_category_ids']:
                    self.catalog[d['dataset_source']][id][aspect_ratio_bucket_id].append(i)
        cat_num = [len(x) for x in self.catalog]

        dataset_weight = [torch.ones(len(self.catalog[i])) * max(cat_num) / c * r / sum(dataset_ratio) \
                          for i, (r, c) in enumerate(zip(dataset_ratio, cat_num))]
        dataset_weight = torch.cat(dataset_weight)

        rfs_factors = []
        for i in range(len(self.catalog)):
            if use_rfs[i]:
                rfs_factor = new_repeat_factors_from_tag_frequency(
                    self.catalog[i], repeat_threshold)
            else:
                rfs_factor = torch.ones(len(self.catalog[i]))
            rfs_factors.append(rfs_factor)
        rfs_factors = torch.cat(rfs_factors)

        self.weights = dataset_weight * rfs_factors
        self.sample_epoch_size = len(dataset_dicts)

        self.idx2dataset = []
        self.idx2cat = []
        self.aspect_ratio_per_cat = []
        for i in range(len(self.catalog)):
            self.idx2dataset += [i] * len(self.catalog[i])
            self.idx2cat += sorted(self.catalog[i].keys())
            for k in sorted(self.catalog[i].keys()):
                v = self.catalog[i][k]
                ratio = len(v[1]) / (len(v[0]) + len(v[1]))
                self.aspect_ratio_per_cat.append(ratio)
    ...
    def _infinite_indices(self):
        ...
        g.manual_seed(self._seed)
        while True:
            cat_ids = torch.multinomial(
                self.weights, self.sample_epoch_size // self.group_size, generator=g,
                replacement=True)
            ratio_prob = torch.tensor([self.aspect_ratio_per_cat[i] for i in cat_ids], dtype=torch.float)
            ratio_bucket = torch.bernoulli(ratio_prob, generator=g)
            ratio_bucket = ratio_bucket.type(torch.int32)

            n = len(cat_ids) // self._world_size * self._world_size
            cat_ids = cat_ids[:n]
            ids = [-1] * (n * self.group_size)
            for i, cat_id in enumerate(cat_ids):
                concept_pool = self.catalog[self.idx2dataset[cat_id]][self.idx2cat[cat_id]][ratio_bucket[i]]
                sampled_group = torch.randint(high=len(concept_pool), size=(self.group_size,), generator=g)
                start = (i // self._world_size) * (self._world_size * self.group_size) + (i % self._world_size)
                for j, id in enumerate(sampled_group):
                    ids[start + self._world_size * j] = concept_pool[id]
            ids = torch.tensor(ids, dtype=torch.long)
            yield from ids


def new_repeat_factors_from_tag_frequency(catalog, repeat_thresh):
    """
    """
    category_freq = defaultdict(float)
    num_images = sum([len(v[0]) + len(v[1]) for k, v in catalog.items()])
    for k, v in catalog.items():
        category_freq[k] = (len(v[0]) + len(v[1])) / num_images

    category_rep = {
        cat_id: max(1.0, math.sqrt(repeat_thresh / cat_freq)) * cat_freq * len(category_freq)
        for cat_id, cat_freq in category_freq.items()
    }

    rep_factors = [category_rep[k] for k in sorted(category_rep.keys())]
    rep_factors = [x / (sum(rep_factors) / len(category_freq)) for x in rep_factors]
    return torch.tensor(rep_factors, dtype=torch.float32)